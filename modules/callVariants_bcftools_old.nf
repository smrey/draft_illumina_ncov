process callVariants_bcftools_old {
    /**
    ...
    * minimum base quality of 0 for a base to be considered, minimum mapping quality of 0 for an alignment to be used.
    * Calls variants from primer trimmed bam using bcftools (http://samtools.github.io/bcftools/bcftools.html)
    * Options: keep all alternate alleles (-A), output blocks of homozygous ref calls (-g)- min sample depth 1,
    * ploidy of 1 (-g 1 option- gVCF??)
    * @input 
    * @output 
    */
    ...
    output:
        tuple dataset_id, path("${dataset_id}_bcftools_old.vcf"), emit: vcf
        path("${dataset_id}*.txt") optional true
    
    script:
        """
        bcftools mpileup -a INFO/AD,FORMAT/AD -d 1000000 -A -f ${ref_genome} -Q 0 -q 0 ${dataset_id}.primertrimmed.sorted.bam \
        | bcftools call -Ac --ploidy 1 -o ${dataset_id}_bcftools_old.vcf \
