process callVariants_ivar {
    ...
    * Ivar options: quality threshold:20, majority/most common base called, call variants if coverage exceeds 1,
    ...
    */
    ...
    publishDir "${params.output}/${task.process}", pattern: '*.fa*', mode: 'copy'

    input:
        tuple(dataset_id, path("${dataset_id}.primertrimmed.sorted.bam"), path("${dataset_id}.primertrimmed.sorted.bam.bai"), path(ref_genome))

    output:
        tuple(dataset_id, path("${dataset_id}.fa")) optional true
        path("${dataset_id}.qual.txt") optional true
    
    script:
        """
        samtools mpileup -A -d 0 -f ${ref_genome} -Q 0 -q 0 ${dataset_id}.primertrimmed.sorted.bam \
        | ivar consensus -p ${dataset_id} -q 20 -t 0 -m 1 -n -
        """
