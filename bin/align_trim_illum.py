...
#Written by Nick Loman edited by Sara Rey Mar 20 to support Illumina paired end reads
#Part of the ZiBRA pipeline (zibraproject.org)

import pysam
import sys
from copy import copy
from read_bed import read_bed_file
...
def trim(args, cigar, s, start_pos, end):
    ...
    # Reads are not discarded using this method, so there should be no issues with paired end sequencing
    # Later on if problem with unpaired reads- sort by read name and look for pairs
    #if s.mate_is_unmapped:
        #raise SystemExit(f"Paired read check failed for {s}")
...
def go(args):
    ...
    # TODO TEMP HARDCODED TO AVOID STDOUT TORRENT OF OUTPUT
    #infile = pysam.AlignmentFile("-", "rb")
    infile = pysam.AlignmentFile("ENT.sorted.bam", "rb")
    #outfile = pysam.AlignmentFile("-", "wh", template=infile)
    outfile = pysam.AlignmentFile("ENT.primersorted.bam", "wh", template=infile)
    ...
    for s in infile:
        ...
        if not correctly_paired:
            print(f"\nCorrectly paired value is {correctly_paired}, read {s}\n")
            print(f"start position is {s.reference_start}")
            raise SystemExit()
